...
let handleCmd = (bot, message) => {
    ...
    let standingsUrl = config['teamInfo']['standings'];
    //local testing
    /* let opts = {
        "playerStats": {
            "index": ["1"],
    ...
    let opts = {
        "playerStats": {
            "index": ["1"],
            "shotSize": {"width": "850", "height": "525"}
        },
        "gameResults": {
            "index": ["0"],
            "shotSize": {"width": "900", "height": "800"}
        },
        "goalieStats": {
            "index": ["2"],
            "shotSize": {"width": "600", "height": "85"}
        },
        "teamStats": {
            "index": ["3", "4"],
            "shotSize": {"width": "480", "height": "480"}
        },
        "standings": {
            "index": [""],
            "shotSize": {"width": "700", "height": "450"}
...
let scrape = (bot, message, url, opts) => {
    ...
    bot.replyPublic(message, `${message.user_name} asked for ${message.text}. Getting screenshot...`);
